...
const props: UploadProps = {
  beforeUpload: (file) => {
    const isPNG = file.type === "application/msword";
    if (!isPNG) {
      message.error(`${file.name} is not a Word file`);
    }
...
const WORDOnly: React.FC<MyProps> = ({ label }) => {
  ...
  return (
    <div>
      <p className="flex text-xl  ">{inputLabel}</p>
      <br />
      <div className="flex">
        <Upload {...props}>
          <Button icon={<UploadOutlined />}>
            Upload word files (.doc) only
          </Button>
        </Upload>
